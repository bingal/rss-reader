...
app.post('/refresh-all', async (c) => {
  ...
  try {
    ...
    const feeds = feedsQuery.all() as { id: string; url: string; title: string }[];
    ...
    for (const feed of feeds) {
      try {
        ...
        const linksQuery = db.query('SELECT link FROM articles WHERE feed_id = ?');
        const existingLinks = new Set(
          (linksQuery.all(feed.id) as { link: string }[]).map(row => row.link)
        );
        ...
        db.query('UPDATE feeds SET updated_at = ? WHERE id = ?').run(now, feed.id);
      } catch (error: any) {
    ...
    if (totalCount === 0 && errors.length > 0) {
      return c.json({ error: `All feeds failed to refresh. First error: ${errors[0]}` }, 400);
    }
    
    return c.json({ count: totalCount, errors: errors.length > 0 ? errors : undefined });
  } catch (error: any) {
